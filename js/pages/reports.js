 ...
 * Analytics dashboard providing summary metrics, state breakdown visual meters, and customer ratios.
 */
...
export const ReportsPage = {
    async render(container) {
        ...
        const maxCustState = Math.max(...Object.values(metrics.customerStateBreakdown || {}), 1);
            ...
            <div class="card" style="margin-bottom: 24px;">
                <div class="card-header">
                    <h3 class="card-title">
                        ...
                        ${isDistributor ? 'Distributor Performance Reports & Analytics' : 'Solar Channel & Customer Network Analytics'}
                    </h3>
                </div>
                ...
                <div class="stats-grid" style="margin-bottom: 24px;">
                    ...
                    <div class="stat-card blue">
                        <div class="stat-info">
                            ...
                            <div style="font-size: 12px; color: var(--slate-500); margin-top: 4px;">
                                Active: <strong>${metrics.activeCustomers}</strong> | Pending: <strong>${metrics.pendingCustomers}</strong>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="stat-card purple">
                        <div class="stat-info">
                            <div class="stat-label">Customer Completion Rate</div>
                            <div class="stat-value">${metrics.totalCustomers ? Math.round((metrics.activeCustomers / metrics.totalCustomers) * 100) : 0}%</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="dashboard-grid" style="${isDistributor ? 'grid-template-columns: 1fr;' : ''}">
                    ...
                    <div class="card" style="border: 1px solid var(--slate-200);">
                        ...
                        <div style="display: flex; flex-direction: column; gap: 14px;">
                            ${Object.keys(metrics.customerStateBreakdown).length === 0 ? '<p class="text-muted">No customer regional data available.</p>' : ''}
                            ${Object.entries(metrics.customerStateBreakdown).map(([state, count]) => {
                                const percent = Math.round((count / maxCustState) * 100);
                                return `
                                    <div>
                                        <div style="display: flex; justify-content: space-between; font-size: 13px; font-weight: 600; margin-bottom: 4px;">
                                            <span>${state}</span>
